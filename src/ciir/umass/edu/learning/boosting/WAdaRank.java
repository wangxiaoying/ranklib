...
/**
 * This class implements the Improved AdaRank algorithm.
 */
public class WAdaRank extends Ranker {
    //Paramters
    ...
    public WAdaRank(List<RankList> samples, int[] features, MetricScorer scorer)
    {
    ...
    private double getScore(RankList rl)
    {
        ...
        return scorer.naiveScore(rl) * samples.size() / sumIdealScore;
    }
    ...
    private WeakRanker learnWeakRanker()
    {
        ...
        for (int i : features) {
            ...
            for (int j = 0; j < samples.size(); j++) {
                double t = getScore(wr.rank(samples.get(j))) * sweight[j];
                s += t;
            }
    ...
    private int learn(int startIteration, boolean withEnqueue)
    {
        ...
        for(; t<=nIteration; t++)
        {
            ...
            for(int i=0;i<samples.size();i++)
            {
                double tmp = getScore(bestWR.rank(samples.get(i)));
            ...
            for (RankList sample : samples) {
                double tmp = getScore(rank(sample));
            ...
            for(int i=0;i<sweight.length;i++)
                sweight[i] *= Math.exp(-alpha_t*getScore(rank(samples.get(i))))/total;
        }
        return t;
    }
    ...
    public void init()
    {
        ...
        // init samples' weight according to their idealScore
        // only need to sum up once
        /*if (sumIdealScore < 0)
        {
            double sum = 0;
        ...
        for (int i = 0; i < sweight.length; ++i)
        {
            sweight[i] = scorer.idealScore(samples.get(i)) / sumIdealScore;
        }*/
    ...
    public Ranker createNew() {return new WAdaRank();}
